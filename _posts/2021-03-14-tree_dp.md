...
public:
    vector<int> sumOfDistancesInTree(int n, vector<vector<int>>& es) {
        ...
        dfs(0, 0);
        ans[0] = accumulate(dep.begin(), dep.end(), 0) - n;
        dfs1(0, 0);
        return ans;
    }
};
...
+ 以u为根，进行dfs，求出在整棵树的根为 u 的情况下最大的路径价值和
+ 将根结点从u换到相邻节点v，这种相邻换根中，除了 u 和 v 这两个结点外，任意其他结点 x 的最大价值路径和都不会发生改变，也就是只有 O(1) 个结点的值发生了改变，换根可以递归进行,最后扩散到整棵树。
...
long long maxOutput(int n, vector<vector<int>>& es, vector<int>& ps) {
    ...
    function<void(int, int)> dfs = [&](int u, int fa) {
        ...
        for (int v : g[u]) if (v != fa) {
            dfs(v, u);
            d[u] = max(d[u], d[v] + ps[u]);
        }
    };

    dfs(0, -1);

    function<void(int, int, long long)> dfs2 = [&](int u, int fa, long long p) {
        ans = max({ans, d[u] - ps[u], p});
        long long mx1 = p, mx2 = p;
        for (int v : g[u]) if (v != fa) {
            if (d[v] > mx1) {
                mx2 = mx1, mx1 = d[v];
            } else if (d[v] > mx2) {
