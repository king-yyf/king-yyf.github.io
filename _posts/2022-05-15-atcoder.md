...
- [atcoder](#atcoder)
  ...
  - [最多奖金数](#最多奖金数)
  - [每次操作后的值](#每次操作后的值)
  - [均值为整数的子序列数量](#均值为整数的子序列数量)
  - [最大长为m乘积子数组](#最大长为m乘积子数组)
  - [最大长为m乘积子序列](#最大长为m乘积子序列)
  - [每次操作后的mex](每次操作后的mex)
...
求满足 `hs < hi < hb` 且 `ws < wi < wb` 的矩形面积之和，矩形不能旋转。
...
二维前缀和，定义二维数组 a[1001][1001], 如果有个长为h,宽为w的矩形， 则 a[h][w] += h * w;
s为a的前缀和，则对于查询hs,ws,hb,wb，从hs+1,ws+1到 hb-1，wb-1之间的矩形和即为答案。
...
长度为n的整数数组，共有 2^n-1个非空子序列，这些子序列中有多少个子序列的平均值是个整数，答案模998244353。
...
[abc 267 c](https://atcoder.jp/contests/abc267/tasks/abc267_c)

长度为n的数组a,求一个长度为m的子数组b，使得 `1*b[1]+2*b[2]+...+m*b[m]` 最大。
...
从i开始的子数组乘积和s[i]与从i+1开始的子数组乘积和s[i+1]的关系为

```
s[i+1]-s[i] = (1*a[i+1]+2*a[i+2]+...+(m-1)*a[i+m-1]+m*a[i+m]) - (1*a[i]+...+m*a[i+m-1])
            = m*a[i+m] -(a[i]+a[i+1]+...+a[i+m-1])
```
...
long long maxKSubarrySum(vector<int> &a, int m) {
    int n = a.size();
    ...
    for (int i = 0; i < n; ++i) {
        if (i < m) s[0] += (i + 1ll) * a[i];
            ...
            s[i - m + 1] = s[i - m] + m * (long long)a[i] - (p[i] - p[i - m]);
        }
    }
...
长度为n的数组a,求一个长度为m的子序列b，使得 `1*b[1]+2*b[2]+...+m*b[m]` 最大。
...
`dp[i][j]`表示：从a数组的前i个元素中选j个元素的最大值。
...
long long maxKSubSequenceSum(vector<int> &a, int m) {
    int n = a.size();
    vector dp(n+1,vector<long long>(n+1, -1e18));
    for (int i = 0; i <= n; ++i) dp[i][0] = 0;
    for (int i = 1; i <= n; ++i) {
        for (int j = 1; j <= i; ++j) {
            dp[i][j] = max(dp[i - 1][j], dp[i - 1][j - 1] + (long long)a[i - 1] * j);
        }
    }
    return dp[n][m];
}
