...
维护 p[i]: 以节点i为起点满足条件的顶点数目，d[i]:从节点i出发，路径上只有0个质数的顶点数目。当计算完子节点v,当前节点是u时，有四种情况

1. v是质数，u也是质数 v 对 u 节点的 p, d 都没有贡献
2. v是质数，u不是质数，所有v满足条件路径上加上u节点，仍然满足条件，且(u,v)也满足条件。p[u]+=p[v]+1
3. v不是质数，u是质数，v为起点的含有0个质数的路径都满足条件，且(u,v)也满足条件，p[u]+=d[v]+1
4. v和u都不是质数, p[u] += p[v], d[u] += d[v] + 1;
