...
[leetcode第50场周赛第四题](https://leetcode-cn.com/problems/maximum-genetic-difference-query/)

**分析** 从根节点开始dfs遍历整颗树，遍历到某个节点时，如果该节点在查询列表里，则计算val与当前树中
已有节点异或的最大值，dfs完某个节点需要将该节点从字典树删除。

```c++
const int N = 1e5 + 10, K = 20, M = K * N;
struct BDTree{
    ...
};
...
public:
    ...
    void dfs(int u) {
        if(u != -1) bt.add(u);
        if (um.count(u)) for (auto& p: um[u]) {
            ans[p[0]] = bt.query(~p[1]);
        }
        for (auto& x : mp[u]) {
            dfs(x);
            bt.del(x);
    ...
    vector<int> maxGeneticDifference(vector<int>& fa, vector<vector<int>>& qs) {
        n = fa.size(), m = qs.size();
        mp.clear(),um.clear();
        ans.resize(m);

        for (int i = 0; i < n; ++i) mp[fa[i]].push_back(i);
        for (int i = 0; i < m; ++i) um[qs[i][0]].push_back({i, qs[i][1]});
