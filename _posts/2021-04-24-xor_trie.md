...
二进制字典树， 对于每一个a[i], 先对其取反，设为x，则与x异或值最大的数为 数组中尽量和x高位相同的数，贪心选取即可。 
...
区间dp，区间[l,r]内的最大值区间端点异或的最大值，或者[l + 1, r] 和 [l, r - 1] 内的最大值取出。

转移方程：f[i][j] = max({a[i] ^ a[j], f[i][j - 1], f[i + 1][j]});
...
```c++
int MaxXorSum(vector<int>& a, int k) {
    int n = a.size(), ans = 0;
    vector<int> s(n + 1);
...
[leetcode第50场周赛第四题](https://leetcode.cn/problems/maximum-genetic-difference-query/)
...
[acwing 256](https://www.acwing.com/problem/content/description/258/)
